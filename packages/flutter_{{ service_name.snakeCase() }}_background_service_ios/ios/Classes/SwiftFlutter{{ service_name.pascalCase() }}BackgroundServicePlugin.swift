...
public class SwiftFlutter{{ service_name.pascalCase() }}BackgroundServicePlugin: FlutterPluginAppLifeCycleDelegate, FlutterPlugin  {
    public static var taskIdentifier = "dev.flutter.{{ service_name.dotCase() }}.background.refresh"
    ...
    public override func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [AnyHashable : Any] = [:]) -> Bool {
        ...
        if #available(iOS 13.0, *) {
            SwiftFlutter{{ service_name.pascalCase() }}BackgroundServicePlugin.registerTaskIdentifier(taskIdentifier: SwiftFlutter{{ service_name.pascalCase() }}BackgroundServicePlugin.taskIdentifier)
        }
    ...
    public func applicationDidEnterBackground(_ application: UIApplication) {
        if #available(iOS 13.0, *){
            SwiftFlutter{{ service_name.pascalCase() }}BackgroundServicePlugin.scheduleAppRefresh()
        }
    }
    ...
    private static func scheduleAppRefresh() {
        let request = BGAppRefreshTaskRequest(identifier: SwiftFlutter{{ service_name.pascalCase() }}BackgroundServicePlugin.taskIdentifier)
       request.earliestBeginDate = Date(timeIntervalSinceNow: 15 * 60)
            
       do {
           // cancel old schedule
           BGTaskScheduler.shared.cancel(taskRequestWithIdentifier: SwiftFlutter{{ service_name.pascalCase() }}BackgroundServicePlugin.taskIdentifier)
    ...
    public static func register(with registrar: FlutterPluginRegistrar) {
        let channel = FlutterMethodChannel(
            name: "id.flutter/{{ service_name.snakeCase() }}_background_service_ios",
            binaryMessenger: registrar.messenger(),
            codec: FlutterJSONMethodCodec())
        
        let instance = SwiftFlutter{{ service_name.pascalCase() }}BackgroundServicePlugin()
...
private class FlutterBackgroundRefreshAppWorker {
    ...
    let uri = "package:flutter_{{ service_name.snakeCase() }}_background_service_ios/flutter_{{ service_name.snakeCase() }}_background_service_ios.dart"
    ...
    public func run() {
        ...
        if (isRunning){
            Flutter{{ service_name.pascalCase() }}BackgroundServicePlugin.register(engine)
            
            let binaryMessenger = engine.binaryMessenger
            channel = FlutterMethodChannel(name: "id.flutter/{{ service_name.snakeCase() }}_background_service_ios_bg", binaryMessenger: binaryMessenger, codec: FlutterJSONMethodCodec())
