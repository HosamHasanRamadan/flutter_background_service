...
public class {{ service_name.pascalCase() }}BackgroundService extends Service implements MethodChannel.MethodCallHandler {
    private static final String TAG = "{{ service_name.pascalCase() }}BackgroundService";
    private static final String LOCK_NAME = {{ service_name.pascalCase() }}BackgroundService.class.getName()
            + ".Lock";
    public static volatile WakeLock lockStatic = null; // notice static
    AtomicBoolean isRunning = new AtomicBoolean(false);
    private FlutterEngine backgroundEngine;
    private MethodChannel methodChannel;
    private {{ service_name.pascalCase() }}Config config;
    ...
    public void onCreate() {
        ...
        Flutter{{ service_name.pascalCase() }}BackgroundServicePlugin.servicePipe.addListener(listener);

        config = new {{ service_name.pascalCase() }}Config(this);
    ...
    public void onDestroy() {
        if (!isManuallyStopped) {
            {{ service_name.pascalCase() }}WatchdogReceiver.enqueue(this);
        } else {
        ...
        Flutter{{ service_name.pascalCase() }}BackgroundServicePlugin.servicePipe.removeListener(listener);
        methodChannel = null;
        dartEntrypoint = null;
        super.onDestroy();
    }

    private final {{ service_name.pascalCase() }}Pipe.PipeListener listener = new {{ service_name.pascalCase() }}Pipe.PipeListener() {
        @Override
    ...
    protected void updateNotificationInfo() {
        if (config.isForeground()) {
            ...
            PendingIntent pi = PendingIntent.getActivity({{ service_name.pascalCase() }}BackgroundService.this, 11, i, flags);
    ...
    private void runService() {
        try {
            ...
            backgroundEngine.getPlugins().remove(Flutter{{ service_name.pascalCase() }}BackgroundServicePlugin.class);

            backgroundEngine.getServiceControlSurface().attachToService({{ service_name.pascalCase() }}BackgroundService.this, null, config.isForeground());

            methodChannel = new MethodChannel(backgroundEngine.getDartExecutor().getBinaryMessenger(), "id.flutter/{{ service_name.snakeCase() }}_background_service_android_bg", JSONMethodCodec.INSTANCE);
            methodChannel.setMethodCallHandler(this);

            dartEntrypoint = new DartExecutor.DartEntrypoint(flutterLoader.findAppBundlePath(), "package:flutter_{{ service_name.snakeCase() }}_background_service_android/flutter_{{ service_name.snakeCase() }}_background_service_android.dart", "entrypoint");
    ...
    public void onTaskRemoved(Intent rootIntent) {
        if (isRunning.get()) {
            {{ service_name.pascalCase() }}WatchdogReceiver.enqueue(getApplicationContext(), 1000);
        }
    }
    ...
    public void onMethodCall(@NonNull MethodCall call, @NonNull MethodChannel.Result result) {
        ...
        try {
            ...
            if (method.equalsIgnoreCase("stopService")) {
                ...
                {{ service_name.pascalCase() }}WatchdogReceiver.remove(this);
                stopSelf();
                result.success(true);
                return;
            }
            ...
            if (method.equalsIgnoreCase("sendData")) {
                try {
                    if (Flutter{{ service_name.pascalCase() }}BackgroundServicePlugin.mainPipe.hasListener()){
                        Flutter{{ service_name.pascalCase() }}BackgroundServicePlugin.mainPipe.invoke((JSONObject) call.arguments);
                    }
