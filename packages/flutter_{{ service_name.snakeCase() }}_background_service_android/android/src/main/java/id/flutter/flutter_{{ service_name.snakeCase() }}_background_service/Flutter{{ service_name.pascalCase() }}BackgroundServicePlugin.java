 ...
 */
public class Flutter{{ service_name.pascalCase() }}BackgroundServicePlugin implements FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler {
    private static final String TAG = "{{ service_name.pascalCase() }}BackgroundServicePlugin";
    ...
    public static final {{ service_name.pascalCase() }}Pipe servicePipe = new {{ service_name.pascalCase() }}Pipe();
    public static final {{ service_name.pascalCase() }}Pipe mainPipe = new {{ service_name.pascalCase() }}Pipe();
    ...
    public void onAttachedToEngine(@NonNull FlutterPluginBinding flutterPluginBinding) {
        ...
        this.config = new {{ service_name.pascalCase() }}Config(this.context);

        mainHandler = new Handler(context.getMainLooper());

        channel = new MethodChannel(flutterPluginBinding.getBinaryMessenger(), "id.flutter/{{ service_name.snakeCase() }}_background_service/android/method", JSONMethodCodec.INSTANCE);
        channel.setMethodCallHandler(this);

        eventChannel = new EventChannel(flutterPluginBinding.getBinaryMessenger(), "id.flutter/{{ service_name.snakeCase() }}_background_service/android/event", JSONMethodCodec.INSTANCE);
    ...
    private void start() {
        {{ service_name.pascalCase() }}WatchdogReceiver.enqueue(context);
        boolean isForeground = config.isForeground();
        Intent intent = new Intent(context, {{ service_name.pascalCase() }}BackgroundService.class);
    ...
    private boolean isServiceRunning() {
        ...
        for (ActivityManager.RunningServiceInfo service : manager.getRunningServices(Integer.MAX_VALUE)) {
            if ({{ service_name.pascalCase() }}BackgroundService.class.getName().equals(service.service.getClassName())) {
                return true;
            }
        }
        return false;
