...
public class {{ service_name.pascalCase() }}WatchdogReceiver extends BroadcastReceiver {
    ...
    public static void enqueue(Context context, int millis) {
        Intent intent = new Intent(context, {{ service_name.pascalCase() }}WatchdogReceiver.class);
    ...
    public static void remove(Context context) {
        ...
        PendingIntent pi = PendingIntent.getBroadcast(context, {{ service_name.pascalCase() }}WatchdogReceiver.QUEUE_REQUEST_ID, intent, flags);
            ...
            if (!config.isManuallyStopped() && !isRunning) {
                try {
                    if (config.isForeground()) {
                        ContextCompat.startForegroundService(context, new Intent(context, id.flutter.flutter_{{ service_name.snakeCase() }}_background_service.{{ service_name.pascalCase() }}BackgroundService.class));
                    } else {
                        context.getApplicationContext().startService(new Intent(context, id.flutter.flutter_{{ service_name.snakeCase() }}_background_service.{{ service_name.pascalCase() }}BackgroundService.class));
                    }}
                catch (Exception e){
                    e.printStackTrace();
                }
